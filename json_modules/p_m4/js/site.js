...
$(function() {
	...
	if($('.JmolPanels').length) {
		...
		var script = 'load pdb_structure.php?id='+$('#Jmol0').attr("value")+";select all; cartoon only; color cartoon structure; spin on;";//+$.getUrlVar('id')+'; cpk off; spin on; wireframe off;cartoon; color cartoon structure;';
...
function initDataTables() {
	...
	$("#browseTable tfoot input").live( "keyup",function () {
	...
	$("#browseTable_wrapper thead input").live( "keyup",function () {
		window.clearTimeout(delayTime);
		var $this = this;
		delayTime = window.setTimeout(function() {
			/* Filter on the column (the index) of this element */
