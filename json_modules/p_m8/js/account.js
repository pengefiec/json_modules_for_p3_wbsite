$(function() 
{
	...
	$("#form_signup").live("submit",function()
	{
		...
		({
			type: "POST",
			url: "account.php", 
			data: {username: $("#username_signup").val(), password1: $("#pass1").val(),password2: $("#pass2").val(), email: $("#signup_email").val(), captcha: $("#signup_captcha").val(), mode: "signup", location:window.location.href},
			success:function()
					{
						$("input.text").removeClass( "ui-state-error" );
						$("span.text").html('');
						...
						{
							$("#load_container" ).html("<div class='loading_success'>Welcome to P3DB!</div>")
							setTimeout(function(){window.location.reload();},1000);
							return true;
						}
						else 
